...
export default function BeatsPage() {
  ...
  /** After onboarding CTA, or first pointer on /beats for returning users (browser autoplay policy). */
  ...
  useEffect(() => {
    if (loading || !autoplayUnlocked || !currentBeat) return;
    const url = currentBeat.audio_preview_url;
    if (url) {
      useAudioStore.getState().play(currentBeat.id, url);
    } else {
  ...
  return (
    <>
      ...
      <div
        ...
        onPointerDown={() => setAutoplayUnlocked(true)}
      >
        {/* Counter — top right */}
        <div
